...
class VBOTaxonomyFinance
{
    ...
    /**
     * Returns the global object, either a new instance,
     * or the previously instantiated instance.
     * 
     * @param   ?array   $options    Optional options to bind.
     *
     * @return  VBOTaxonomyFinance
     */
    public static function getInstance(?array $options = null)
    {
        if (is_null(static::$instance)) {
            static::$instance = new static((array) $options);
        }
     ...
     * @param   string  $type   one among "stay_dates", "booking_dates" or "checkin".
     * 
     * @return  array           associative list of information.
     * 
     * @throws  Exception
     * 
     * @since   1.18.13 (J) - 1.8.13 (WP) added support to $type = "checkin".
     */
    public function getStats($from, $to, array $rooms = [], $type = 'stay_dates')
    {
        ...
        if ($type == 'stay_dates') {
            ...
            // calculation based on check-in date
        ...
        foreach ($bookings as $bid => $booking) {
            ...
            foreach ($booking as $room_booking) {
                ...
                if ($this->options['booking_level'] ?? null) {
                    ...
                    $stats['_bid_stats'][$bid]['tot_vat'] += (float) $room_tot_taxes;
                    $stats['_bid_stats'][$bid]['city_taxes'] += (float) $room_tot_city_taxes;
                    $stats['_bid_stats'][$bid]['damage_deposits'] += (float) $room_tot_damage_dep;
                    $stats['_bid_stats'][$bid]['cmms'] += (float) $room_cmms;
                    $stats['_bid_stats'][$bid]['tot_refunds'] += $room_refund;
                }
            }
