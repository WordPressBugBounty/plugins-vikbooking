...
/**
 * ISTAT Sitra è valido per diverse regioni. Sicuramente per l'Abruzzo.
 */
class VikBookingReportIstatSitra extends VikBookingReport
{
	/**
	...
	public function __construct()
	{
		...
		$this->reportName = JText::translate('ISTAT Sitra');
				...
				foreach ($row as $field) {
					...
					if ($type == 1) {
						...
						if ($field['key'] == 'turismo') {
							$clienti['arrivi'][$arrivi]['tipoturismo'] = $export_value;
						}
						if ($field['key'] == 'mezzo') {
							$clienti['arrivi'][$arrivi]['mezzotrasporto'] = $export_value;
						}
						...
						if (isset($clienti['arrivi'][$arrivi]['cittadinanza']) && $clienti['arrivi'][$arrivi]['cittadinanza'] == '100000100' && $field['key'] == 'comres') {
							$clienti['arrivi'][$arrivi]['luogoresidenza'] = (string)$export_value;
						}
						if (isset($clienti['arrivi'][$arrivi]['tipoalloggiato']) && ($clienti['arrivi'][$arrivi]['tipoalloggiato'] == '19' || $clienti['arrivi'][$arrivi]['tipoalloggiato'] == '20') && $field['key'] == 'tipo' ) {
							$clienti['arrivi'][$arrivi]['idcapo'] = (string)$idswh;
						}
					} elseif ($type == 2) {
